...
현재 위치에 개행 시퀀스가 있는지 확인합니다. 개행 시퀀스가 없다면 `_pe` 결과로 종료합니다. 개행 시퀀스가 있다면, 개행 문자 이후의 첫 문자로 커서를 이동합니다. 공백을 무시하지 않습니다. 개행 시퀀스는 `\r\n` = 13 10 혹은 `\n` = 10 중 하나입니다.
...
현재 위치가 EOF인지 확인합니다. 현재 위치가 EOF가 아니라면 `_pe` 결과로 종료합니다.
