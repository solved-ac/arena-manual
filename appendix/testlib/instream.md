...
- [목차](#목차)
- [inf, ouf, ans](#inf-ouf-ans)
- [주요 입력 함수](#주요-입력-함수)
- [Strict mode](#strict-mode)
- [일반](#일반)
  - [InStream::skipBlanks](#instreamskipblanks)
  - [InStream::curChar](#instreamcurchar)
  - [InStream::skipChar](#instreamskipchar)
  - [InStream::readChar](#instreamreadchar)
  - [InStream::nextChar](#instreamnextchar)
  - [InStream::readSpace](#instreamreadspace)
  - [InStream::unreadChar](#instreamunreadchar)
  - [InStream::eof](#instreameof)
  - [InStream::seekEof](#instreamseekeof)
  - [InStream::eoln](#instreameoln)
  - [InStream::seekEoln](#instreamseekeoln)
  - [InStream::nextLine](#instreamnextline)
  - [InStream::readEoln](#instreamreadeoln)
  - [InStream::readEof](#instreamreadeof)
- [토큰](#토큰)
  - [InStream::readToken](#instreamreadtoken)
  - [InStream::readTokens](#instreamreadtokens)
  - [InStream::readTokenTo](#instreamreadtokento)
  - [InStream::readWord](#instreamreadword)
  - [InStream::readWords](#instreamreadwords)
  - [InStream::readWordTo](#instreamreadwordto)
  - [InStream::readLine](#instreamreadline)
  - [InStream::readLines](#instreamreadlines)
  - [InStream::readLineTo](#instreamreadlineto)
  - [InStream::readString](#instreamreadstring)
  - [InStream::readStrings](#instreamreadstrings)
  - [InStream::readStringTo](#instreamreadstringto)
- [정수](#정수)
  - [InStream::readInt](#instreamreadint)
  - [InStream::readInts](#instreamreadints)
  - [InStream::readInteger](#instreamreadinteger)
  - [InStream::readIntegers](#instreamreadintegers)
  - [InStream::readLong](#instreamreadlong)
  - [InStream::readLongs](#instreamreadlongs)
  - [InStream::readUnsignedLong](#instreamreadunsignedlong)
  - [InStream::readUnsignedLongs](#instreamreadunsignedlongs)
- [실수](#실수)
  - [InStream::readDouble](#instreamreaddouble)
  - [InStream::readDoubles](#instreamreaddoubles)
  - [InStream::readReal](#instreamreadreal)
  - [InStream::readReals](#instreamreadreals)
  - [InStream::readStrictDouble](#instreamreadstrictdouble)
  - [InStream::readStrictDoubles](#instreamreadstrictdoubles)
  - [InStream::readStrictReal](#instreamreadstrictreal)
  - [InStream::readStrictReals](#instreamreadstrictreals)
...
`c`가 주어진 경우, 읽은 문자가 `c`가 아닐 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
현재 위치에 개행 시퀀스가 있는지 확인합니다. 개행 시퀀스가 없다면 [`_pe`](tresult.md#_pe) 결과로 종료합니다. 개행 시퀀스가 있다면, 개행 문자 이후의 첫 문자로 커서를 이동합니다. 공백을 무시하지 않습니다. 개행 시퀀스는 `\r\n` = 13 10 혹은 `\n` = 10 중 하나입니다.

Strict mode에서는 Windows 환경일 경우에는 `\r\n`만을 개행 시퀀스로 보며, 그렇지 않은 환경일 경우에는 `\n`만을 개행 시퀀스로 봅니다.

### InStream::readEof

- `void readEof()`

현재 위치가 EOF인지 확인합니다. 현재 위치가 EOF가 아니라면 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 현재 위치가 공백임에도 불구하고 `readToken`을 수항하는 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 토큰과 토큰 사이에는 무조건 한 개의 띄어쓰기(` ` = 32)가 있어야 하고, 띄어쓰기 이외의 공백 문자는 허용되지 않습니다. 현재 위치가 공백 문자임에도 불구하고 `readTokens`을 수행하는 경우, 또는 토큰이 띄어쓰기 한 개보다 많은 수의 공백 문자로 구분된 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
정수는 0을 제외하고서는 `0`으로 시작할 수 없습니다. 음수의 경우에도 `-0`으로 시작할 수 없습니다. 십진수 표현만 허용되며, 과학적 표기법, 예를 들어 `1e8` 등은 허용되지 않습니다. 만약 읽은 정수가 명시한 제한을 만족하지 않는다면 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 현재 위치가 공백임에도 불구하고 `readInt`을 수항하는 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 토큰과 토큰 사이에는 무조건 한 개의 띄어쓰기(` ` = 32)가 있어야 하고, 띄어쓰기 이외의 공백 문자는 허용되지 않습니다. 현재 위치가 공백 문자임에도 불구하고 `readInts`을 수행하는 경우, 또는 토큰이 띄어쓰기 한 개보다 많은 수의 공백 문자로 구분된 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 현재 위치가 공백임에도 불구하고 `readLong`을 수항하는 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 토큰과 토큰 사이에는 무조건 한 개의 띄어쓰기(` ` = 32)가 있어야 하고, 띄어쓰기 이외의 공백 문자는 허용되지 않습니다. 현재 위치가 공백 문자임에도 불구하고 `readLongs`을 수행하는 경우, 또는 토큰이 띄어쓰기 한 개보다 많은 수의 공백 문자로 구분된 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 현재 위치가 공백임에도 불구하고 `readUnsignedLong`을 수항하는 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 토큰과 토큰 사이에는 무조건 한 개의 띄어쓰기(` ` = 32)가 있어야 하고, 띄어쓰기 이외의 공백 문자는 허용되지 않습니다. 현재 위치가 공백 문자임에도 불구하고 `readUnsignedLongs`을 수행하는 경우, 또는 토큰이 띄어쓰기 한 개보다 많은 수의 공백 문자로 구분된 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
실수는 C의 `%lf`로 입력받을 수 있는 형식 중 숫자가 한 개 이상 등장하는 형식만을 허용합니다. 과학적 표기법, 예를 들어 `3.1415e2` 등은 **허용됩니다**. 실수의 형식이 올바르지 않다면 [`_pe`](tresult.md#_pe) 결과로 종료합니다. 허용되는 예는 다음과 같습니다.
...
Strict mode에서는 공백을 무시하지 않으며, 현재 위치가 공백임에도 불구하고 `readDouble`을 수항하는 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 토큰과 토큰 사이에는 무조건 한 개의 띄어쓰기(` ` = 32)가 있어야 하고, 띄어쓰기 이외의 공백 문자는 허용되지 않습니다. 현재 위치가 공백 문자임에도 불구하고 `readDoubles`을 수행하는 경우, 또는 토큰이 띄어쓰기 한 개보다 많은 수의 공백 문자로 구분된 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
실수는 `[0-9]*(\.[0-9]+)?` 형태여야 하며, 소숫점 이하에 $\left[\texttt{minAfterPointDigitCount}, \texttt{maxAfterPointDigitCount}\right]$개의 숫자를 가져야 합니다. 과학적 표기법, 예를 들어 `3.1415e2` 등은 **허용되지 않으며**, 첫 글자가 소수점인 경우도 허용되지 않습니다. 실수의 형식이 올바르지 않다면 [`_pe`](tresult.md#_pe) 결과로 종료합니다. [readDouble](#instreamreaddouble)과 입력 형식이 다름에 주의하며, 입력 함수는 소수점 이하 숫자의 수에 제한을 걸 뿐 소수점 이하 숫자의 수가 일정 이상 많아져도 `double`의 정밀도를 넘어서는 경우의 정밀도를 보장하지는 않음에 주의합니다.

`minv`와 `maxv`를 명시하는 경우, $\texttt{minv}\_\mathbb{F} \le x \le \texttt{maxv}\_\mathbb{F}$를 만족하는 실수 $x$를 읽습니다. 만약 읽은 실수가 명시한 범위를 만족하지 않는다면 `_wa` 결과로 종료합니다.

Strict mode에서는 공백을 무시하지 않으며, 현재 위치가 공백임에도 불구하고 `readStrictDouble`을 수항하는 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
...
Strict mode에서는 공백을 무시하지 않으며, 토큰과 토큰 사이에는 무조건 한 개의 띄어쓰기(` ` = 32)가 있어야 하고, 띄어쓰기 이외의 공백 문자는 허용되지 않습니다. 현재 위치가 공백 문자임에도 불구하고 `readStrictDoubles`을 수행하는 경우, 또는 토큰이 띄어쓰기 한 개보다 많은 수의 공백 문자로 구분된 경우 [`_pe`](tresult.md#_pe) 결과로 종료합니다.
